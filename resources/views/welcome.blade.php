...
<body class="bg-gray-50 text-slate-600 font-sans antialiased">
    ...
    @php
        // CRITICAL FIX: Database se published pages fetch karein
            ...
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    ...
                    @foreach($staticPages as $page)
                        <a href="/{{ $page->slug }}" class="text-sm font-medium text-slate-600 hover:text-primary transition">
                            {{ $page->title }}
                        </a>
                    @endforeach

                    <a href="/admin/login" class="bg-primary hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition shadow-md shadow-indigo-200">
                        Login
                    </a>
                </div>
            </div>
